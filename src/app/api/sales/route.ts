...
export async function POST(request: Request) {
    try {
      ...
      const { date, clientId, amount, method, note, clientRate } = body;
  
      // Get default rate from client if not provided
      let finalRate = clientRate;
      if (finalRate === undefined) {
        const client = await prisma.rates.findUnique({
          where: { id: clientId },
          select: { rate: true },
        });
        finalRate = client?.rate;
      }
...
export async function GET() {
    try {
      ...
      // Format the response to include client name and rate
      const formattedSales = sales.map((sale) => ({
        ...
        method: sale.method,
        note: sale.note,
        clientRate: sale.clientRate ?? sale.client.rate, // Use clientRate if set, otherwise use client.rate
      }));
  ...
  try {
    ...
    // Handle batch deletion
      ...
      for (let i = 0; i < totalBatches; i++) {
        ...
        console.log(
          `Processed deletion batch ${i + 1}/${totalBatches} (${totalDeleted}/${
            ids.length
          } items)`
        );
      }
